...
## 다관점 패스 (보조 — SUGGESTION만 생성)

7점은 엔지니어링 결함을 잡는다. 이 패스는 **비엔지니어링 관점**에서 놓쳤을 만한 것을 짚는다.
**CRITICAL/MINOR로 격상하지 말고 항상 SUGGESTION으로만 보고하라.** 발견 없으면 생략.

- **PO/제품**: 변경이 사용자에게 어떤 가시적 차이를 만드는가? 이슈 의도(왜)와 결과(무엇)가 어긋나지 않는가?
- **운영**: 배포 시 롤백 가능한가? 마이그레이션 순서/다운타임 고려가 있는가? 모니터링/알림 추가가 필요한 변경인가?
- **QA**: 회귀 위험 영역(인접 기능)에 영향이 없는가? 테스트가 행동을 검증하는가, 구현 디테일을 검증하는가?
