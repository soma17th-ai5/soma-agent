...
from soma_app.clients import OpenAIClient, UpstageClient
from soma_app.models.schemas import EmbeddingResult, LayoutAnalysisResult, EmbeddingContext, EmbeddingContextList
...
class EmbeddingService:
    ...
    async def rag(self, messages: List[str], collection: str = "embeddings", model: str='solar-embedding-1-large-query') -> Optional[EmbeddingContextList]:
        """
        ...
        async with get_chrome_client() as client:
            collection_obj: Collection = await client.get_collection(collection)
            result: QueryResult = await collection_obj.query(
